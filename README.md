# Olist Data Warehouse Project

Choose your language / Elige tu idioma:

- [Español](./README.es.md)
- [English](./README.en.md)

This repository documents an end-to-end data project built with the public Olist dataset, Azure Data Factory, SQL Server, and a layered architecture with `raw`, `stg`, and `dw` schemas.

Este repositorio documenta un proyecto de datos de extremo a extremo construido con el dataset publico de Olist, Azure Data Factory, SQL Server y una arquitectura por capas con esquemas `raw`, `stg` y `dw`.
